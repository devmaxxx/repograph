...
answer costs. On the bench fixture a fused query is ~0.30 s and ~1.4 GB — the model, not the graph
— of which 220 ms is the open itself: a cache lookup, then the 16 MB tokenizer and the 448 MB ONNX
session opening concurrently. It was 410 ms until the cache lookup stopped asking the hub for a
weights file the small model has never had and waiting out the 404, which is also what makes the
dense arm work offline. An exact-id lookup answers in ~50 ms and ~50 MB, and a `--no-dense`
