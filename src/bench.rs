...
mod tests {
    ...
    #[test]
    fn floors() {
        let s = Summary { keyword: (24, 24), paraphrase: (3, 14), code: (3, 3), p90_tokens: 220 };
        assert!(passes(&s, false));
        assert!(!passes(&s, true));
        assert!(passes(&Summary { paraphrase: (5, 14), ..s.clone() }, true));
        assert!(!passes(&Summary { p90_tokens: 231, ..s.clone() }, false));
        assert!(!passes(&Summary { keyword: (23, 24), ..s.clone() }, false));
        assert!(!passes(&Summary { code: (2, 3), ..s.clone() }, false));
    }
