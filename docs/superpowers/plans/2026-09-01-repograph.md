...
~~None of these changes the accuracy gates.~~ False for the tantivy row: reproducing the BM25-over-bodies prototype's configuration on the shipped hand-rolled index scores 2/14 paraphrase where tantivy recorded 4/14 — see [`docs/adr/ADR-001-paraphrase-recall-was-a-prediction.md`](../../adr/ADR-001-paraphrase-recall-was-a-prediction.md). The rkyv and petgraph rows are unaffected. If Max wants rkyv/tantivy/petgraph back, the store and lexical modules are the only files that change.
