...
**Tracking:** #51 in the [0.6.0 milestone](https://github.com/devmaxxx/repograph/milestone/2), blocked by #49, the language layer.
...
- **The corpus is read in its clone, never in `~/Documents/projects`.** Every block that touches it starts `CORP=$(cat ~/bench/corpora-src/bonliva-crm-nx.path); P=~/bench/corpora-private/bonliva-crm-nx`. Every reading runs in the pin with the branch binary and `export REPOGRAPH_CODE_GLOBS='**/*.ts **/*.tsx **/*.js **/*.jsx **/*.mjs **/*.cjs **/*.cs **/*.razor **/*.cshtml'` (master Task 9; the JavaScript globs are `main`'s defaults since 0.5.3, #64); no `repograph.toml` is ever written into the pin, and the store under `$CORP/.repograph` is the only thing a reading leaves there.
...
- A `.razor` whose first bytes are a UTF-8 BOM (Visual Studio's default) with `@code {` or `@namespace`/`@using`/`@inject` on line 1 → the block is read and its members declared, and the first-line directive is read; the scanner starts after the BOM. Task 7 (`a_bom_before_the_first_line_is_not_part_of_it`), Task 8 (`a_bom_before_a_first_line_directive_is_not_part_of_it`).
- A BOM'd `.cs` or `.razor` on the truth side (`﻿public class A`) → line 1's type or directive is counted, because the blankers drop the BOM before the `^[ \t]*`-anchored regexes read it. Task 9 (`test_a_bom_does_not_hide_the_first_line`).
- Declarations under `#if`/`#else`, at file level and inside a type → declared like any other (`B`, `A.Trace`), as the truth's line regexes count them. Task 2 (`declarations_under_a_preprocessor_branch_are_declared`).
- `public delegate void PlacedHandler(int x);` → `PlacedHandler` is a symbol and `PlacedHandler.x` is not; a delegate's parameter list declares no member. Task 2 (`a_delegate_parameter_is_not_a_member`).
- A BOM + CRLF `.cs` and a CRLF `.razor` → the header's directives and `top` are the plain file's, a member's row is its line, and the blanked Razor copy keeps every byte and newline and parses clean. Task 3 (`bom_and_crlf_keep_rows_and_the_header`), Task 7 (`a_crlf_file_keeps_every_row_and_the_wrapper`).
...
- `CodeExtractor::extract`'s dispatch on `lang::Lang::of(rel)`, whose last arm is `Some(other) => unreachable!`, and the shared node dedup and edge sort after it.
...
In `src/code/lang.rs`, add to `Lang::of`'s match on `ext(rel)`, an `Option<&str>`:
...
In `src/code/mod.rs`, beside the other module lines, `pub mod csharp;` and `pub mod razor;`, and in the dispatch's `match lang::Lang::of(rel)`, above the `#[allow(unreachable_patterns)] Some(other) => unreachable!(…)` arm:
...
| `#if X … #else … #endif` around declarations | `preproc_if` holding the `#if` branch's declarations as its own children, with the `#else` branch nested in it as `preproc_else`; at file level and inside a `declaration_list` alike |
...
fn a_delegate_parameter_is_not_a_member() {
    let ids = ids(&one("Shop/H.cs", "namespace Shop;\npublic delegate void PlacedHandler(int x);\n"));
...
and, in `collect`, below the master's `if let Some(header) = header_for(..) { index_header(..) }` (the master's `collect` has no match on `lang`; Task 8 turns this `if` into one when Razor joins):
...
In `src/code/index.rs`, `header_for` matches on `lang.family()`, and `Family::DotNet` sits in the shared name-indexed `=> None` arm. Take it out of that arm and give it its own, matching on `lang`:
...
The arm reads both parameters, so the master's `let _ = (rel, source);` (or the `let _ = rel;` the JVM plan narrows it to, if that plan landed first) is dead and is deleted.

`grep -n 'fn collect\|fn collect_manifest\|fn header_for' -A12 src/code/imports.rs src/code/index.rs` shows the master's forms; the lines go in as written, and nothing else in either function changes.
...
Expected: PASS. The first test is the one L3 exists for; if it fails with `Checkout.cs` missing, run `cargo run -- --no-dense --repo <tempdir> update` on a copy and diff its `.repograph/headers.json` before and after to see whether `widen` added the family (`update` prints no widen detail, and `REPOGRAPH_TIMING` is read only by `ask` and `watch`), and the fault is in the `header_for` C# arm (Task 3), never in `widen`.
...
In `src/code/imports.rs`, the `if matches!(lang, Lang::CSharp)` Task 3 put in `collect` becomes a match with the Razor arm:
...
and in `src/code/index.rs`, inside `header_for`'s `Family::DotNet => match lang { … }` from Task 3, above `_ => None`:
...
Two existing cases assume `.cs` has no reader, and both break once `CSHARP_DI` and the C# declarations register; each moves its fixture to an extension no 0.6.0 plan registers and keeps its assertions:
- `test_a_file_no_reader_reads_is_in_the_diff_and_in_neither_axis`: `b.cs` becomes `b.zig` in all four places.
- `TypeScriptTruthStandsAlone.OTHER`: `"apps/Order.cs"` becomes `"apps/Order.zig"`, same body. The case proves other languages' files change nothing in the TypeScript graph; with `.cs` read, `Order → IPay.Charge` would join the graph and the equality would fail for a reason the case is not about.

This plan is the first to merge after the master, so those are the registrations `main` holds when it branches: `JS_FAMILY`'s six from 0.5.3, then the master's `.kt` declarations.
...
**Files:**
- Modify: `docs/bench/2026-09-11-0.6.0-results.md` (the reading and verdict cells of `## L · C#` and `## L · Razor`, the Razor parse row's "reads" cell, and a cost paragraph under each)
...
export REPOGRAPH_CODE_GLOBS='**/*.ts **/*.tsx **/*.js **/*.jsx **/*.mjs **/*.cjs **/*.cs **/*.razor **/*.cshtml'
```
...
Expected: Task 1's `CSharp` row (`bad` ≤ 1) and Task 7's `razor-census` line (`unread + unclean` ≤ 1). These are the two parse cells. `## L · Razor`'s parse row says it reads `parse_census`, which has no Razor grammar to count; Razor is counted by `razor_block_census`, so this step also rewrites that row's "reads" cell to "`razor_block_census` unread + unclean ≤ §0's count", before the number goes beside it.
...
`run.py` prints each tool's summary; that goes to `$P`, and only the aggregates below reach the log. The corpus defines no id families, so the run reads the shipped cache with `--id-families`; that pattern feeds only the retrieval rank, never blast scoring. The Razor `changes` case diffs about 2,900 files (Task 10), so the timeout is raised for every call.

```bash
D=$(date +%F)
python3 bench/compare/run.py --repo $CORP --repograph $NEW --tools repograph --suites blast \
  --blast $P/blast.jsonl --truth $P/truth.json --timeout 900 --id-families bench/compare/id-families.json \
  --out $P/results-$D.json > $P/run.out 2>&1; echo "run.py exit $?"
git -C $CORP status --porcelain -- . ':!.repograph' | wc -l
...
Last, because it empties the store: the build from nothing, on main's default globs alone and with .NET, in one sitting.

```bash
for i in 1 2 3; do rm -rf $CORP/.repograph; REPOGRAPH_CODE_GLOBS='**/*.ts **/*.tsx **/*.js **/*.jsx **/*.mjs **/*.cjs' bench/probe/measure.sh build-ts-$i $L -- $NEW --repo $CORP --no-dense build; done
wc -c < $CORP/.repograph/graph.json | awk '{print "graph.json bytes, TypeScript and JavaScript only:", $1}'
...
Behind an explicit glob until the defaults task: `REPOGRAPH_CODE_GLOBS='**/*.ts **/*.tsx **/*.js **/*.jsx **/*.mjs **/*.cjs **/*.cs **/*.razor **/*.cshtml'`, or the same in `code_globs`.
