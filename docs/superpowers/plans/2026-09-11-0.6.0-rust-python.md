...
`bonliva-erp` is employer code. The selection reads the pin, `$ERP`, whose commit `$PRIV/pin.txt` records, so every anchor it writes is the pin's (a stale anchor is what `bench` refuses since 0.5.2, #35); Step 6 checks that first. The rule that picks its cases is committed; the cases, their truth and every result stay in `~/bench/corpora-private/bonliva-erp/`. The script and its verification print counts only. What is known without naming anything: the repository tracks 12 `.py` files holding 102 definitions, and 10 of those files sit under a dotted top-level directory. `build`'s walk reads dotted paths since 0.5.3 (#64), so the store holds all 12; `truth.rg` walks as ripgrep does by default and reads the 2 undotted ones only. The selection therefore keeps to undotted files and refuses a name a dotted file spells. The planning probe read no `self.x = T(…)` chain in the 2. So a `trace` case will most likely be waived, and the script records the waiver rather than inventing a case (spec §5).
...
class Select(unittest.TestCase):
    ...
    def test_impact_takes_names_every_referencing_file_imports(self):
        ...
        self.assertNotIn("connect", impact, "a dotted file spells it: the store reads that file, the truth does not")
...
def visible(repo: Path, ext: str) -> list[str]:
    # ripgrep's default walk, which `truth.rg` shares: ignored and dotted paths left out. `build`
    # reads dotted paths since 0.5.3 (#64), so this is a subset of what the store holds, never more.
    return sorted(T.rg(repo, ["--files", "-g", f"*{ext}"]))


def dotted_naming(repo: Path, name: str, decl: str) -> bool:
    # The store reads a dotted file and the truth does not, so a name one spells would be scored
    # against refs the truth cannot see.
