...
fn a_globbed_file_no_grammar_reads_is_named_once_on_stderr() {
    ...
    let repo = dir.path();
    write(repo, "repograph.toml", "code_globs = [\"**/*.ts\", \"**/*.kt\"]\n");
    write(repo, "web/a.ts", "export function boot() {}\n");
    write(repo, "svc/Program.kt", "class Program\n");
    write(repo, "svc/Order.kt", "class Order\n");
    let out = common::run(repo, &["build"]);
    let err = String::from_utf8_lossy(&out.stderr);
    assert!(out.status.success(), "{err}");
    assert_eq!(err.matches("code: no grammar reads 2 .kt — indexed as files only").count(), 1, "{err}");
}
