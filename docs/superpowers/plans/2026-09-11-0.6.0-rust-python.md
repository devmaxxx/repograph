...
**Tracking:** #53 in the [0.6.0 milestone](https://github.com/devmaxxx/repograph/milestone/2), blocked by #49, the language layer.
...
- **The embedder is declared in the environment.** `REPOGRAPH_EMBED_MODEL=intfloat/multilingual-e5-small` is exported before any `bench/probe/` script. `reset.sh` and `embedder.sh` refuse without it; `measure.sh`, the one this plan calls, does not check, so the preamble's export is what pins it. Every build this plan measures is `--no-dense`.
...
D='**/*.ts **/*.tsx **/*.js **/*.jsx **/*.mjs **/*.cjs'  # main's six default globs; every reading adds its language's to these
```

## Review Focus

1. `use crate::Store;` in `src/ops.rs`, where `src/lib.rs` holds `pub use store::Store;`, then `Store::new()` → one `Calls` edge to `sym:src/store.rs::Store.new` and none to `sym:src/lib.rs::Store.new`, an id no node has. Task 4 (`a_path_through_a_crate_root_re_export_lands_on_the_declaring_file`), Task 5 (`a_call_through_a_crate_root_re_export_lands_on_the_declaring_file`).
2. `#[cfg(unix)] fn rename_over() {}` and a `#[cfg(windows)] fn rename_over() {…}` twin in one file → one `sym:src/lib.rs::rename_over` node whose span runs from the first twin's name to the second twin's close (2–7), so a hunk in the second twin is a change to it. Task 3 (`cfg_twins_are_one_symbol_whose_span_covers_both`).
3. `self.exec.sleep(5).await` and `let _ = self.exec.sleep(6).await;` with `exec: E` and `E: Exec + Clone` → `Calls` to `sym:src/exec.rs::Exec.sleep`; the `.await` changes nothing. Task 5 (`an_awaited_call_through_a_typed_field_resolves`).
4. `from shop import Store` where `shop/__init__.py` holds `from .store import Store` → `Store()` and `Store.open(…)` call `sym:shop/store.py::Store` and `sym:shop/store.py::Store.open`; no edge targets `sym:shop/__init__.py::…`. Task 9 (`a_name_re_exported_by_a_package_init_resolves_to_its_declaring_module`).
5. `@retry(3)` on a function and `@retry(times=2)` on a method, `retry` imported from a sibling module → `DecoratedBy` to `sym:tools/retry.py::retry` with context `retry` from each; the call's arguments name nothing. Task 9 (`a_decorator_called_with_arguments_decorates_with_its_callee`).
...
- `REPOGRAPH_CODE_GLOBS` (master Task 9): whitespace-separated globs that replace `code_globs` for one run. Every reading here exports it as `main`'s six defaults (`$D`) plus its language's globs; no `repograph.toml` is written into a clone and no special reading binary is built.
...
The two modules and the skeletons the cases call go in now, so the cases compile and Step 3 fails on what Step 4 adds rather than on a missing file. `src/code/mod.rs`, beside the master's module lines:
...
Expected: the four new tests compile and FAIL — both `a_…_file_is_one_file_node_and_parses_clean` on `Lang::of(…)` left `None` (no `.rs` or `.py` arm yet), both `every_kind_the_walk_matches_is_in_the_grammar` on `grammar().unwrap()` of `None` (no grammar crate yet). The skeleton `extract`s are unused until Step 4's dispatch arms; the dead-code warning goes with them.

- [ ] **Step 4: Add the grammars and the arms**
...
```rust
// in `Lang::of`, the match on the extension, an `Option<&str>`:
...
`src/code/mod.rs`, in `CodeExtractor::extract`'s match, above the `Some(other) => unreachable!` arm (the module has no `use` for `Lang`, so the arms spell it as the master's TypeScript arm does):
...
Expected, one block per root, the language as `parse_census` prints it (`{lang:?}`, padded to 11): `repograph` (at `b21fb0f`) `Rust 43 0 0 0` and `Python 22 0 0 0`; `agent-orchestrator` `Rust 116 0 0 0`; `bonliva-erp` a `Python` row with `bad 0`. The columns are padded, so read them by field, not by the literal spacing. Spec §0 counted 41 `.rs` files in `repograph` on `50a042f`; `v0.5.0` adds `tests/exit_codes.rs` and `tests/common/mod.rs`, hence 43. `bonliva-erp` tracks 16 `.py` files at its pin, 14 of them under a dotted top-level directory (the counts only; no names). `walk.rs` reads dotted paths since 0.5.3 (#64) and prunes only `.git`, so a census that walks as `build` does reads all 16; the gate is `bad`, which must be 0 either way. The planning probe read all three pins at 0 files with an `ERROR` or `MISSING` node.
...
```bash
gh auth switch --hostname github.com --user devmaxxx && git push -u origin feat/0.6.0-rust-python
...
Twins under `#[cfg(unix)]` and `#[cfg(windows)]` — `repograph`'s pin has them in `serve.rs`, `enrich.rs` and `store.rs` — share one id. Two nodes with one id would leave the extractor's dedup keeping the first, and a hunk in the second twin credited to nothing in `changes`; so the second twin widens the first's span and writes no node of its own. The truth reader still counts a twin pair as two declarations: a handful on the pin, inside the ±5% the *declarations* clause allows, and `changed_symbols` credits a hunk in either twin to the one name, as the store now does.
...
Run: `cargo test --bin repograph -- rust_lang:: 2>&1 | tail -20`
Expected: the six new tests FAIL — no `Symbol` node is written yet (`sym:src/lib.rs::Line not extracted` and the like); Tasks 1–2's tests still pass.
...
Run: `cargo test --bin repograph -- rust_lang:: && cargo test && cargo clippy --all-targets -- -D warnings`
Expected: every Rust case so far passes, the twins case included; the suite green; clippy clean.
...
**Interfaces:**
- Produces: `uses::Ctx { rel, src, crates, items, bindings }` with `Ctx::resolve(inline, path) -> Option<Target>`; `uses::Bindings`; `uses::read`; `calls::impls`; `calls::id_of`; `Crates::declares(file, name)`; `Crates::resolve` following one `pub use` hop; `inline_of` in `mod.rs`.
...
A path that lands on a name a file re-exports — `use crate::Store` where `src/lib.rs` holds `pub use store::Store;`, the usual crate-root shape — would name `sym:src/lib.rs::Store`, which no node has, and `impact Store` would miss the caller. The line scan that reads top-level names also reads column-0 `pub use a::b::C;` lines (braces and globs are not followed), and `Crates::resolve` follows such a re-export one hop to the file that declares the name. A chain of re-exports stops after that hop.
...
Run: `cargo test --bin repograph -- rust_lang:: 2>&1 | tail -20`
Expected: compile error on `Crates::declares`; with that stubbed, the five new Rust cases FAIL (no `Imports`, `ReExports`, `References` or `Extends` written), and so does `a_path_through_a_crate_root_re_export_lands_on_the_declaring_file` (`crate::Store` lands on `src/lib.rs`).

- [ ] **Step 3: Top-level names and re-exports in the line scan**
...
rename Task 2's `pub fn resolve` to `fn resolve_from(&self, rel: &str, inline: &[String], path: &[String], hop: bool) -> Option<Target>`, keep its body, and put the old name in front of it:
...
In `resolve_from`, replace the last line, `Some(Target::Item { file, name: item_name(&scope, rest) })`, with:
...
`resolve_from` drops Task 2's `///` comment, which the wrapper now carries. Add a method:
...
Run: `cargo test --bin repograph -- rust_lang:: 2>&1 | tail -20`
Expected: the six new cases FAIL — no `Calls` and no `DecoratedBy` edge is written yet.
...
```bash
gh auth switch --hostname github.com --user devmaxxx && git push
gh auth switch --hostname github.com --user devmaxxx && gh run watch "$(gh run list --workflow ci --branch feat/0.6.0-rust-python --limit 1 --json databaseId --jq '.[0].databaseId')"
```
...
A module name is a path from a root, and the roots are, in the order tried: the importing file's own directory (a script's `sys.path[0]`, which is how every sibling import in `repograph`'s `bench/` resolves — `import truth as T` in `bench/compare/run.py`), each directory holding `pyproject.toml`, `setup.py` or `setup.cfg` above the importing file, nearest first, then the repository root. `a.b` names `a/b.py`, else `a/b/__init__.py`. A project root also tries its `src/` when the package is not beside the manifest (Max, 2026-09-24): in a `pyproject.toml` + `src/pkg/` project, `import pkg` from `tests/` resolves to `src/pkg/__init__.py`, as it does once the project is installed. Only a manifest's directory gets that second try; the manifest itself is still not read, so a `package-dir` that names another directory resolves nothing.
...
Expected: compile errors — `Modules` and `Resolver::python` not found. (With the table written but `src_layout` left out, `a_src_layout_project_is_imported_by_its_package_name` fails on its first assert, `None`.)
...
Expected: the four new tests pass and every Rust case still does (the hooks were rewritten as matches); the suite green; clippy clean.
...
**Interfaces:**
- Produces: `refs::read(resolver_modules, rel, src, root, defs, ex)` writing `Imports`, `Calls`, `Extends` and `DecoratedBy`; `Modules::{init, reexport}`.
...
Run: `cargo test --bin repograph -- python:: 2>&1 | tail -20`
Expected: the six new tests FAIL — no `Imports`, `Calls`, `Extends` or `DecoratedBy` is written yet.

- [ ] **Step 3: Implement the reference pass**

`src/code/python/modules.rs`: `use std::collections::{BTreeMap, BTreeSet};` and `use std::sync::LazyLock;`, and above `Modules`:
...
`src/code/imports.rs`, the Python arm of `collect`:
...
`bonliva-erp` is employer code. The selection reads the pin, `$ERP`, whose commit `$PRIV/pin.txt` records, so every anchor it writes is the pin's (a stale anchor is what `bench` refuses since 0.5.2, #35); Step 6 checks that first. The rule that picks its cases is committed; the cases, their truth and every result stay in `~/bench/corpora-private/bonliva-erp/`. The script and its verification print counts only. What is known without naming anything: the repository tracks 16 `.py` files at its pin, and 14 of those files sit under a dotted top-level directory; Step 6 re-counts the definitions at the pin and prints the count only. `build`'s walk reads dotted paths since 0.5.3 (#64), and `truth.rg` does too since #73 put `--hidden` into `truth.EXCLUDE`. **`Resolver::new`'s walk does not** (spec L3; master Task 10: `hidden(true)`), so `Modules` holds only the undotted files and no import between dotted files resolves. The selection keeps to undotted files for that reason and refuses a name a dotted file spells. The planning probe read no `self.x = T(…)` chain in the 2 undotted files. So a `trace` case will most likely be waived, and the script records the waiver rather than inventing a case (spec §5).

The rule, per kind:

- **impact:** a top-level name declared in exactly one walk-visible file of the extension, not starting with `_` or `test`, spelled in no dotted file, whose truth refs are non-empty, all of the same extension, and each of which imports the declaring module (`import m`, `from m import`, `from .m import`, `import pkg.m`). Tier `wide` at 5 referencing files or more, else `narrow`; each case carries `"exts": ["<ext>"]`. Candidates are ordered by referencing files descending, then name; the first `wide` and the first two `narrow` are taken.
...
class Select(unittest.TestCase):
    ...
    def test_impact_takes_names_every_referencing_file_imports(self):
        ...
        self.assertEqual(impact["Store"], {"kind": "impact", "target": "Store", "file": "tools/store.py", "tier": "narrow", "exts": [".py"]})
...
[ "$(git -C "$ERP" rev-parse HEAD)" = "$(cat $PRIV/pin.txt)" ] && echo "at the pin"
...
files = T.rg(repo, ["--files", "-g", "*.py"])
dotted = sum(any(p.startswith(".") for p in f.split("/")) for f in files)
defs = sum(len(T.declarations(f, (repo / f).read_text(encoding="utf8", errors="replace"))[1]) for f in files)
print("py files", len(files), "dotted", dotted, "definitions", defs)
PY
wc -l < $PRIV/waivers.txt
```

Expected: `at the pin`, `0` dirty lines; one count line from the script; three verification lines each ending `True` (a kind with 0 cases reads `True` vacuously and has a waiver line instead); `py files 16 dotted 14` and the definitions count at the pin, which the results document may quote as a number; the waiver count. Nothing printed names a file or a symbol. `truth.build`'s `roots=["."]` reads the whole tree `truth.rg` walks, dotted directories included; an empty list would fall back to `DI_ROOTS`. The file `$PRIV/waivers.txt` holds each waiver's reason; the results document takes only its line count and the kinds waived.
...
Expected: `Rust 43 0 0 0` and `Python 22 0 0 0` for `repograph`, `Rust 116 0 0 0` for `agent-orchestrator` (columns padded), and `bad 0` for `bonliva-erp`. Clause *parse*: bad ≤ §0's count, which is 0 for both languages on every pin.
...
build3 build-repograph "$RG" "$D **/*.rs **/*.py" $L
build3 build-agent-orchestrator "$AO" "$D **/*.rs" $L
build3 build-erp "$ERP" "$D **/*.py" $PRIV > /dev/null
...
```bash
REPOGRAPH_CODE_GLOBS="$D **/*.rs **/*.py" python3 bench/compare/run.py --repo "$RG" --repograph $NEW --tools repograph --suites blast \
...
REPOGRAPH_CODE_GLOBS="$D **/*.rs" python3 bench/compare/run.py --repo "$AO" --repograph $NEW --tools repograph --suites blast \
...
REPOGRAPH_CODE_GLOBS="$D **/*.py" python3 bench/compare/run.py --repo "$ERP" --repograph $NEW --tools repograph --suites blast \
...
- **Limits the readings stand on**, one line each: `#[path]` modules and calls inside macro arguments are not read; a Rust struct whose `impl` is in another file is not read for field-typed calls, by the extractor or by the truth; a Python local's type is not inferred; 14 of `bonliva-erp`'s 16 `.py` files sit under a dotted directory: extracted, but their imports do not resolve (the resolver's walk skips dotted directories, spec L3).
...
| Rust: visibility, `#[cfg(test)]` read; `#[cfg]` twins as one symbol | Task 3 |
| Rust corpora `repograph`, `agent-orchestrator` | Task 12 |
| Python: module name from the nearest root; `src/` layout below a project manifest (Max, 2026-09-24) | Task 7 |
